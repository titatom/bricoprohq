...
const SS_TABS = [
  ...
  { key: 'queues', label: 'Processing Queues' },
];
...
function QueueTable({ items, onStatusChange, statusOptions, emptyMsg }) {
  if (items.length === 0)
...
function ProcessingQueuesTab({ apiFetch }) {
  ...
  const loadImages = useCallback(async () => {
    ...
    try {
      ...
      else setImgError('Failed to load image queue.');
    } catch { setImgError('Failed to load image queue.'); }
  ...
  const loadDocs = useCallback(async () => {
    ...
    try {
      ...
      else setDocError('Failed to load document queue.');
    } catch { setDocError('Failed to load document queue.'); }
  }, [apiFetch, docStatus, docSource]);

  useEffect(() => { loadImages(); }, [imgStatus, imgSource]); // eslint-disable-line
  useEffect(() => { loadDocs(); }, [docStatus, docSource]); // eslint-disable-line
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="font-semibold text-gray-800">Processing Queues</h3>
        <p className="text-sm text-gray-500 mt-0.5">Review pending photos and documents from Immich and Paperless.</p>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`px-5 py-2.5 text-sm font-medium border-b-2 transition-colors ${
              tab === key
                ? 'border-accent-500 text-brand-600 font-semibold'
...
export default function SocialStudioPage() {
  ...
  return (
    <div className="p-6 max-w-7xl">
      ...
      {activeTab === 'queues' && (
        <ProcessingQueuesTab apiFetch={apiFetch} />
      )}
    </div>
  );
}
